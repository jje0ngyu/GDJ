...
public class SpringBeanConfig {
	...
	/*
	 	@Bean
	 	" 안녕, 난 Bean을 만드는 메소드야!      *   java       xml
	 	  메소드 이름이 Bean의 이름(id)이고,	* 메소드명 = <Bean>의 id
	 	  반환 타입이 Bean의 타입(class)지. "   * 클래스명 = <Bean>의 class
	 */
	...
	public Song 아무입력값1() { // 메소드 이름을 아무거나 적어도 된다.
		Song song = new Song();
		song.setTitle("제목2");
		song.setGenre("장르2");
		return song;
	 ...
	 */
	@Bean			// @Bean에 name값을 지정하면 id로 사용된다.
	public Song song3() { // 메소드 이름을 아무거나 적어도 된다.
