...
<script>
	...
	function fn_add(){
		$('#btn_add').click(function(){
			...
			$.ajax({
				...
				data: member,	// 파라미터 이름 없음(즉, 본문에 member를 포함시켜서 전송)
				contentType : 'application/json',	// 요청 데이터의 MINE-TYPE 으로 작성
				/* 응답 */
				dataType : 'json',	// 부트에서는 json은 SpringWeb에 포함되어 있다.
	...
	function fn_init() {
		...
		$('#name').val('');
		$(':radio[name=gender]').prop('checked', false);	// 체크된 radio 체크해지하기
		$('#address').val('');
	}
	...
	function fn_list() {
		$.ajax({
			type: 'get',
			/* url : '${contextPath}/members?page=' + page,   <<< RESTful에서는 이렇게 적지 않는다!*/
	...
	function fn_modify() {
		$('#btn_modify').click(function() {
			...
			let member = JSON.stringify({
				...
				// id의 경우 readonly로 막았기 때문에 수정할 필요가 없다.
				name: $('#name').val(),
				gender: $(':radio[name=gender]:checked').val(),
				address: $('#address').val()
			});
	...
	function fn_remove() {
		$('#btn_remove').click(function(){
			if(confirm('선택한 회원을 모두 삭제할까요?')){
				...
				// 마지막 콤마(,)를 삭제해 주기 위한 코드
				memberNoList = memberNoList.substr(0, memberNoList.length - 1)	// 3, 1 (마지막 콤마 자름)
				$.ajax({
					type: 'delete',
