-- 그룹(GROUP BY)
-- 1. GROUP BY절에서 지정한 칼럼의 데이터는 하나로 모아서 한 번만 조회된다.
...
SELECT
      DEPARTMENT_ID
    , SUM(SALARY) AS 부서별_연봉합계
    , FLOOR(AVG(SALARY)) AS 부서별_연봉평균
    , MAX(SALARY) AS 부서별_최고연봉
    , MIN(SALARY) AS 부서별_최저연봉
    , COUNT(*) AS 부서별_사원수
  FROM
      EMPLOYEES
 GROUP BY
...
-- 4. 조건 지정
--    1) GROUP BY로 처리할 행(ROW)은 적을 수록 성능 향상
--    2) WHERE절  : GROUP BY 이전에 처리되므로 가능한 모든 조건은 WHERE절에서 처리
--    3) HAVING절 : WHERE절에서 처리할 수 없는 조건만 HAVING절에서 처리

-- 1) 부서번호가 100 미만인 부서들의 연봉평균을 조회하기 (WHERE절, HAVING절 모두 가능)
-- WHERE절 : 성능이 더 우수한 쿼리
SELECT
      DEPARTMENT_ID
    , FLOOR(AVG(SALARY)) AS 부서별_연봉평균
...
-- 2) 소속된 사원 수가 10명 이상인 부서의 연봉평균 조회하기 (HAVING절)
--    소속된 사원 수는 GROUP BY 이후에만 알 수 있기 때문에 WHERE절로 처리가 불가능
SELECT
      DEPARTMENT_ID
    , COUNT(*) AS 부서별_사원수
    , FLOOR(AVG(SALARY)) AS 부서별_연봉평균
...
-- EMPLOYEES 테이블 연습.
-- 1. 급여평균이 10,000이상인 부서의 부서번호와 급여평균을 조회하기
...
--    즉, 부서별 / 직업아이디별로 그룹화하기
--    부서별 직업아이디로 그룹화하여 각 그룹의 사원수 조회하기
--    부서번호가 없는 사원은 제외하기
SELECT
       DEPARTMENT_ID
     , JOB_ID
