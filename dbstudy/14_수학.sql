...
-- 5. 부호 판별
--    SIGN(A) : A의 부호가 양수(+)면 1, 음수(-)면 -1, 0이면 0을 반환
...
-- 9. 원하는 자릿수로 반올림
--    ROUND (A, [DIGIT]) : 실수 A를 DIGIT 자릿수로 절사, DIGIT 생략하면 정수로 반올림
SELECT
      ROUND (145.45)     -- 145
...
--    4) 정수       : CEIL(값 * 1)     / 1  (*? 굳이 그냥 CEIL(값) 하면 될 듯하다.)
--    5) 일의 자리  : CEIL(값 * 0.1)   / 0.1
--    6) 십의 자리  : CEIL(값 * 0.01)  / 0.01
--    7) 백의 자리  : CEIL(값 * 0.001) / 0.001
