...
-- 3. 단위(년, 월, 시, 분, 초) 추출 함수
--    EXTRACT(단위 FROM 날짜)
SELECT
       EXTRACT(YEAR   FROM SYSDATE) AS 연도
     , EXTRACT(MONTH  FROM SYSDATE) AS 월
     , EXTRACT(DAY    FROM SYSDATE) AS 일
     , EXTRACT(HOUR   FROM SYSTIMESTAMP) AS 시 -- UTC 기준 (세계 표준시, 한국 : UTC+9)
...
SELECT
    ...
    , TO_CHAR(SYSDATE -0.5, 'MM/DD AM HH:MI:SS') AS "12시간 전"
    , TO_CHAR(SYSDATE +0.5, 'MM/DD AM HH:MI:SS') AS "12시간 후"
    , TO_CHAR(SYSDATE + ( 1 / 24 ), 'MM/DD AM HH:MI:SS') AS "1시간 후"
  FROM
      DUAL;
...
-- 6. 경과한 개월 수
--    MONTHS_BETWEEN (최근 날짜, 이전 날짜) : 두 날짜 사이에 경과한 개월 수
SELECT
      NAME, MONTHS_BETWEEN(SYSDATE, HIRE_DATE)
  FROM
      EMPLOYEE;
