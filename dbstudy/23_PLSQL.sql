...
BEGIN
    SNO := '901010-1234567';
    SELECT SUBSTR(SNO, 8, 1)  -- 오른쪽에서 인덱스 셀 때 : SUBSTR(SNO, -7, 1);
      INTO GENDER_NUM
      FROM DUAL;
    CASE
...
--    (3) DEPARTMENT_ID가 50이면 DEPT50에 INSERT
...
BEGIN
    I := 0;
    FOR V_ROW IN (SELECT * FROM EMPLOYEES) LOOP
        SALARTIES(I) := V_ROW.SALARY; -- PL/SQL에서 배열은 괄호()로 선언
        I := I + 1;
    END LOOP;    
    
    FOR I IN 0 .. 106 LOOP
        DBMS_OUTPUT.PUT_LINE(SALARIS(I));
    END LOOP;
END;
...
EXCEPTION
     WHEN NO_DATA_FOUND THEN
        DBMS_OUTPUT.PUT_LINE('조회된 데이터 없음');
     WHEN OTHERS THEN    
        DBMS_OUTPUT.PUT_LINE('예외코드 ' || SQLCODE);
        DBMS_OUTPUT.PUT_LINE('예외메시지 ' || SQLERRM); --에러 메시지 출력
END;
