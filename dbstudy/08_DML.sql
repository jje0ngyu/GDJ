...
CREATE TABLE DEPARTMENT (
    DEPT_NO   NUMBER            NOT NULL, --PK
    DEPT_NAME VARCHAR2(15 BYTE) NOT NULL,
    LOCATION  VARCHAR2(15 BYTE) NOT NULL
);
-- EMPLOYEE 테이블
CREATE TABLE EMPLOYEE (
    EMP_NO    NUMBER            NOT NULL, --PK, 사원번호
    NAME      VARCHAR2(20 BYTE) NOT NULL,
    DEPART    NUMBER, --FK (DEPT,NO) 소속부서 번호
...
-- 외래키
ALTER TABLE EMPLOYEE ADD CONSTRAINT FK_EMPLOYEE_DEPARTMENT FOREIGN KEY (DEPART)
...
--* 비어있는 칼럼에는 NULL 값을 넣는다.
--* DEPARTMENT의 LOCATION은 NOT NULL 이므로, 예외 발생
 ...
 */
 /*
    DEPARTMENT의 부서번호를 EMPLOYEE가 참조중이므로 수정 불가
    해결책
    1. 외래키 일시 중지
    2. 수정
    3. 외래키 재시작
