...
/*
    CHAR (size) : 고정 길이 문자 타입 ( 1 ~ 2000 바이트 ) 
    VARCHAR2 (size) : 가변 길이 문자 타입 ( 1 ~ 4000 바이트 )
    NUMBER (p, s) : 정밀도(p), 스케일(s)로 표현되는 숫자 타입
        - 정밀도(p) : 정수 + 소수점 모두를 포함하는 전체 유효 숫자 ( 0은 제외)
        - 스케일(s) : 소수점 자리 수
        예시)
        NUMBER :  최대 38자리 숫자 ( 22 바이트 )
        NUMBER(3) : 최대 3자리 정수 (123) (* 스케일X)
        NUMBER(5, 2) : 최대 전체 5자리 , 소수점 2자리 실수 (123.45) (* 스케일 O) 
                                                            (12345.67) (X)
        : 1 미만의 소수점 2자리 실수   (EX. 0.25)                                                  
        NUMBER(2.2)  (* 백분율 타입에서 주로 사용된다.) 
*/
...
-- 1. 제약조건 이름 없이 테이블 만들기 (1)
-- 오라클에서 DROP과 CREATE는 함께 다닌다. 이미 테이블이 생성된 상태에서 재실행하면 에러가 발생하기 때문!
DROP TABLE USER_TBL;  
