...
SELECT
  ...
  FROM DEPARTMENTS D INNER JOIN EMPLOYEES E
    ON D.DEPARTMENT_ID = E.DEPARTMENT_ID
 GROUP BY D.DEPARTMENT_ID, D.DEPARTMENT_NAME /* 같은 홍보부 (NAME) 이지만, 역할에 따라 다른 부서(ID)일 수 있다. */
 ORDER BY 사원수;
...
-- EMPLOYEES   테이블 : EMPLOYEE_NAME, LAST_NAME
-- LOCATIONS   테이블 : CITY
...
SELECT
       ME.DEPARTMENT_ID AS 내_부서번호
     , ME.LAST_NAME     AS 내_이름
     , ME.SALARY        AS 내_급여
     , ME.HIRE_DATE     AS 내_입사일
     , YOU.LAST_NAME    AS 다른_사원_이름
     , YOU.SALARY       AS 다른_사원_급여
     , YOU.HIRE_DATE    AS 다른_사원_입사일
  FROM
