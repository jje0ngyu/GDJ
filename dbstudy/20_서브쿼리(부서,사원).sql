...
/*
    ...
    6. 서브쿼리 결과에 따른 구분
       1) 단일행 서브쿼리 (SINGLE ROW)
          (1) 서브쿼리 결과가 1개
          (2) PK나 UNIQUE 칼럼의 동등 비교(=) 결과   EX) EMPLOYEE_ID = 100
          (3) 함수의 결과 (SUM, AVG, MAX, MIN)
          (4) 단일 행 연산자를 사용 (=, !=, >, >=, <, <=)
          
       2) 다중행 서브쿼리
          (1) 서브쿼리 결과가 2개 이상
          (2) FORM절(인라인뷰), WHERE절에서 사용
          (3) 다중 행 연선자를 사용 (IN, ANY, ALL 등)
*/

-- 1. 사원번호가 1001인 사원과 같은 직급(POSITION)을 가진 사원 조회하기
SELECT EMP_NO, NAME, DEPART, GENDER, POSITION, HIRE_DATE, SALARY
  FROM EMPLOYEE
 WHERE POSITION = (SELECT POSITION        -- 단일행 서브쿼리이므로 연산자 '='를 사용, 하위쿼리는 메인쿼리와 동등비교(=) 되므로 반드시 POSITION을 반환
                     FROM EMPLOYEE
                    WHERE EMP_NO = 1001); -- EMP_NO 는 PK이므로 단일행 서브쿼리
...
SELECT EMP_NO, NAME
  FROM EMPLOYEE
 WHERE ROWID = 'AAAE95AABAAALDBAAC';
...
-- 2. ROWNUM은 1을 포함하지 않는 범위는 조건을 사용할 수 없다.
SELECT EMP_NO, NAME
  FROM EMPLOYEE
...
SELECT A.EMP_NO, A.NAME, A.HIRE_DATE
  FROM (SELECT RANK() OVER (ORDER BY HIRE_DATE) AS 순위, EMP_NO, NAME, HIRE_DATE
          FROM EMPLOYEE) A
...
CREATE TABLE DEPARTMENT2 
    AS (SELECT DEPT_NO, DEPT_NAME, LOCATION
          FROM DEPARTMENT
         WHERE 1 = 2); --* 절대 만족하지 않는 조건을 줌으로써 행 제외, 구조만 복사된다.
