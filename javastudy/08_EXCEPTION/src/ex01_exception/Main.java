...
//				   hobbies[i].equals("수영") && hobbies[i] != null    은 서로 다른 코드이다.
				System.out.println("취미가 나와 같군요");
			}
		}
	...
	public static void m3() {
		...
		//* "오백원" → 500 으로 반환을 요구할 때 발생
		
		Scanner sc = new Scanner(System.in);
		System.out.println("이름 입력(필수) >>> ");
		String name = sc.nextLine();
		System.out.println("나이 입력(선택) >>> ");
		String strAge = sc.nextLine();	// 입력 없이 Enter만 누르면 strAge는 빈 문자열을 가진다.
		
		int age = Integer.parseInt(strAge);	//* 나이를 숫자로 반환
