...
/*
 	...
 	1. 웹 화면을 만드는 자바 클래스
 	2. JSP/Servlet Container(Tomcat)에 의해서 실행 (* 자바 버추얼 머신에서 실행 X)
 	3. 서블릿을 실행하면 웹 브라우저가 실행된다.
 	4. URL
 	 	프로토콜://호스트:포트번호/컨텍스트패스/URPMapping
 	 	* ContextPass : Project 주소 (같은 Project면 같은 값, 다른 값이면 서로 다른 프로젝트) 
 	 	* URPMapping  : Servlet 주소 (같은 Servlet이면 같은 값)
 	ex)	http://localhost:9090/01_Servlet/HelloServlet
*/
...
public class HelloServlet extends HttpServlet {	// Servlet은 HttpServlet을 상속 받으면 된다.
	...
	/*
	    2. 초기화
	    	1) 각종 초기화 정보 실행
	    	2) 서블릿 환경 설정 처리 	(* config : 환경설정 = conf )
	...
	/*
	 	3. 서비스
	 		1) 클라이언트의 요청마다 매번 호출
	 		2) GET 방식의 요청은 doGet() 메소드 호출, POST 방식의 요청은 doPost() 메소드 호출	(* 직접 호출 X)
	 */
	...
	/*
	 	4. doGet
	 		1) GET 방식의 요청을 처리
	 		2) 요청 메소드가 GET인 경우		(* 주소를 이용한 방식은 대부분 "GET" 방식이다.)
	 	  ajax  (1) $.ajax({
	 					'type': 'GET',
	 					'url' : 'http://localhost:9090/01_Servlet/HelloServlet'
	 				});
	 	   CSS	(2) <a href="http://localhost:9090/01_Servlet/HelloServlet">링크</a>
	 			(3) <form method="GET" action="http://localhost:9090/01_Servlet/HelloServlet"> 또는 <form>
	 		JS	(4) location.href='http://localhost:9090/01_Servlet/HelloServlet'
