...
<script>
	...
	function fn_init(){	
		...
		$('#name').val('');
		$(':radio[name=gender]').prop('cheched', false);
		$('#grade').val('');
		$('#address').val('');
	}
	...
	function fn_registration(){
		...
		$('#btn_add').click(function(){
			...
			$.ajax({
				...
				data: $('#frm_member').serialize(),	// serialize() : 폼의 모든 입력 요소를 파라미터로 변환 (name 속성을 파라미터로 전환)
				/* 응답 */
				dataType: 'json',
				// 정상 응답
				...
				// 예외 응답
				error: function(jqXHR){	// 예외 처리 응답 데이터(일반 텍스트)는 jqXHR 객체의 responseText 속성에 저장된다.
